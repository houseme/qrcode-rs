...
fn bench_encode(c: &mut Criterion) {
    ...
    g.bench_function("short", |b| b.iter(|| QrCode::new(black_box(b"hello")).unwrap()));
    g.bench_function("medium", |b| b.iter(|| QrCode::new(black_box(&medium)).unwrap()));
    g.bench_function("long", |b| b.iter(|| QrCode::new(black_box(&long)).unwrap()));
    ...
    for v in [1_i16, 10, 20, 30, 40] {
        g.bench_function(format!("v{v}"), |b| {
            b.iter(|| QrCode::with_version(black_box(b"hello"), Version::Normal(v), EcLevel::L).unwrap())
        });
    }
    g.finish();
}
