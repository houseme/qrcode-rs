//! Structured Append bit-stream parser — the pure-core decode counterpart to
//! [`StructuredAppend::encode`](crate::structured_append::StructuredAppend).
//!
...
/// [`SaSymbol`](crate::structured_append::SaSymbol)s from each symbol's
/// `position` / `total` / `parity` / `data` and pass them to
/// [`reassemble`](crate::structured_append::reassemble).
...
/// the push-side [`Bits`](crate::bits::Bits).
struct BitReader<'a> {
    data: &'a [u8],
    pos: usize,
}
...
/// # Errors
///
/// Returns [`SaError::NotStructuredAppend`] if the stream does not begin with
/// the Structured Append mode indicator (`0011`), or [`SaError::MalformedStream`]
/// if the bits run out mid-field.
